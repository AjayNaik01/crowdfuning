...
const EditCampaign = () => {
    ...
    useEffect(() => {
        ...
        if (!isNaN(target) && target >= 50000) {
            setFormData(prev => ({ ...prev, isVotingEnabled: true }));
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                setFormData({
                    ...
                    isVotingEnabled: c.isVotingEnabled || false,
                    isOrganization: c.isOrganization || false,
                    organizationName: c.organizationName || '',
                    organizationDetails: c.organizationDetails || ''
                });
                setExistingImages((c.images || []).map(img => typeof img === 'string' ? { url: img } : img));
                setExistingVideos((c.videos || []).map(vid => typeof vid === 'string' ? { url: vid } : vid));
            } else {
                alert(data.message || 'Failed to load campaign');
    ...
    const handleAddVideoClick = () => videoInputRef.current.click();
    ...
    const handleRemoveVideo = (idx, isExisting) => {
        if (isExisting) setExistingVideos(prev => prev.filter((_, i) => i !== idx));
        else setVideos(prev => prev.filter((_, i) => i !== idx));
    };
    const handleFileChange = (e) => {
        const { name, files } = e.target;
        if (name === 'images') setImages(prev => [...prev, ...Array.from(files)]);
        else if (name === 'videos') setVideos(prev => [...prev, ...Array.from(files)]);
    };
    ...
    const validateStep = () => {
        ...
        if (currentStep === 1) {
            ...
            if (existingImages.length === 0 && existingVideos.length === 0 && images.length === 0 && videos.length === 0) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            form.append('existingVideos', JSON.stringify(existingVideos));
            const response = await fetch(`http://localhost:5001/api/campaigns/${id}`, {
                method: 'PUT',
            ...
            const data = await response.json();
            if (data.success) {
                alert('Campaign updated successfully! Redirecting...');
                        ...
                        <div className="w-full max-w-3xl bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl border border-blue-100 p-8 flex flex-col gap-10 items-center relative overflow-visible">
                            <div className="w-full flex flex-col md:flex-row gap-10 md:gap-16 items-start justify-center">
                                    ...
                                    {(existingImages.length > 0 || images.length > 0) && (
                                        <div className="w-full mt-4 flex gap-3 overflow-x-auto pb-2">
                                            {existingImages.map((img, idx) => (
                                                <div key={`existing-img-${idx}`} className="relative group animate-fade-in">
                                                    <img src={img.url} alt={`existing-preview-${idx}`} className="w-24 h-24 object-cover rounded-xl border border-blue-200 shadow-md" />
                                                    <button type="button" onClick={() => handleRemoveImage(idx, true)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-base opacity-80 group-hover:opacity-100 shadow-lg" title="Remove">×</button>
                                                </div>
                                            ))}
                                            {images.map((img, idx) => (
                                                <div key={`new-img-${idx}`} className="relative group animate-fade-in">
                                                    <img src={URL.createObjectURL(img)} alt={`preview-${idx}`} className="w-24 h-24 object-cover rounded-xl border border-blue-200 shadow-md" />
                                                    <button type="button" onClick={() => handleRemoveImage(idx, false)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-base opacity-80 group-hover:opacity-100 shadow-lg" title="Remove">×</button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                ...
                                <div className="flex-1 w-full flex flex-col items-center">
                                    ...
                                    {/* Previews for existing and new videos */}
                                    {(existingVideos.length > 0 || videos.length > 0) && (
                                        <div className="w-full mt-4 flex gap-3 overflow-x-auto pb-2">
                                            {existingVideos.map((vid, idx) => (
                                                <div key={`existing-vid-${idx}`} className="relative group animate-fade-in">
                                                    <video src={vid.url} controls className="w-28 h-24 object-cover rounded-xl border border-blue-200 shadow-md" />
                                                    <button type="button" onClick={() => handleRemoveVideo(idx, true)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-base opacity-80 group-hover:opacity-100 shadow-lg" title="Remove">×</button>
                                                </div>
                                            ))}
                                            {videos.map((vid, idx) => (
                                                <div key={`new-vid-${idx}`} className="relative group animate-fade-in">
                                                    <video src={URL.createObjectURL(vid)} controls className="w-28 h-24 object-cover rounded-xl border border-blue-200 shadow-md" />
                                                    <button type="button" onClick={() => handleRemoveVideo(idx, false)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-base opacity-80 group-hover:opacity-100 shadow-lg" title="Remove">×</button>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                    ...
                    <div className="w-full max-w-[80vw] rounded-3xl shadow-2xl bg-white/60 backdrop-blur-lg border border-indigo-100 p-8 relative flex flex-col justify-center">
                        ...
                        <form onSubmit={handleSubmit} className="mt-8" encType="multipart/form-data">
                            ...
                            <div className="flex justify-between mt-12 gap-4 sticky bottom-0 bg-transparent z-20">
                                ...
                                {currentStep === stepper.length && (
                                    <button type="submit" disabled={isSubmitting} className="bg-indigo-600 text-white px-6 py-2 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 transition-all">{isSubmitting ? 'Updating...' : 'Update Campaign'}</button>
                                )}
                            </div>
                        </form>
                    </div>
